if (window.Settings && window.Settings.__hackos) {
  ...
  /**
   * Composant pour la fenetre Settings
   */
  class Settings extends Window {
    ...
    initializeSettings() {
      const title = HTMLBuilder.build('h1', {
        innerText: tr('settings.title', 'Settings'),
        style: 'font-size: 3vw; line-height: 1; margin: 0.4vw 0 0.1vw 0;',
      });

      const description = HTMLBuilder.build('p', {
        innerText: tr('settings.description', 'Choose the website language.'),
